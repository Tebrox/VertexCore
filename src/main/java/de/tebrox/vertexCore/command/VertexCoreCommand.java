...
public final class VertexCoreCommand implements CommandExecutor, TabCompleter {

    private static final List<String> ROOT = List.of("migrate");
    ...
    public VertexCoreCommand(Plugin corePlugin, PluginDataRegistry registry) {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!sender.hasPermission("vertexcore.migrate")) {
        ...
        if (!BACKENDS.contains(from) || !BACKENDS.contains(to)) {
            sender.sendMessage("Backends: json | h2 | mysql");
            return true;
        }

        PluginDataRegistry.Entry entry = registry.get(pluginName);
        if (entry == null) {
            sender.sendMessage("Plugin is not registered for migration: " + pluginName);
            return true;
        }

        Plugin targetPlugin = Bukkit.getPluginManager().getPlugin(pluginName);
        if (targetPlugin == null || !targetPlugin.isEnabled()) {
            sender.sendMessage("Plugin not found or not enabled: " + pluginName);
            return true;
        }
        ...
        if (wantsDelete && !confirmed) {
            sender.sendMessage("Refusing to delete source data without confirmation.");
            sender.sendMessage("Add --confirm to proceed: /" + label + " migrate " + pluginName + " " + from + " " + to + " --delete-source --confirm");
            return true;
        }

        sender.sendMessage("Starting migration: plugin=" + targetPlugin.getName() + " from=" + from + " to=" + to
                + (opt.dryRun ? " (dry-run)" : ""));
        ...
        // async via queue (wenn du queueFor nicht willst, nimm api.asyncExecutor())
        api.databaseService()
                ...
                .whenComplete((v, err) -> Bukkit.getScheduler().runTask(corePlugin, () -> {
                    if (err != null) {
                        ...
                        sender.sendMessage("Migration failed: " + (u.getMessage() == null ? u.getClass().getSimpleName() : u.getMessage()));
                        u.printStackTrace();
                    } else {
    ...
    private static void usage(CommandSender sender, String label) {
        sender.sendMessage("Usage: /" + label + " migrate <plugin> <from> <to> [--dry-run] [--overwrite] [--delete-source]");
        sender.sendMessage("Backends: json | h2 | mysql");
    }
...
// flags
        if (args.length >= 5) {
            ...
            for (int i = 4; i < args.length; i++) used.add(args[i].toLowerCase());
    ...
    private static List<String> filter(List<String> options, String token) {
        ...
        return options.stream()
                .filter(o -> o.toLowerCase().startsWith(t))
                .collect(Collectors.toList());
    }
}
